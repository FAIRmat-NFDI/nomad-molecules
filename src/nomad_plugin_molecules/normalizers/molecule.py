...
from .atoms_utils import create_atoms_object, validate_atom_count, validate_dimensionality, query_molecule_database_util, generate_topology_util
...
class MoleculeNormalizer(Normalizer):
    ...
    def normalize(self, archive: EntryArchive, logger=None) -> list:
        ...
        try:
            ...
            if not validate_atom_count(atoms_data, self.min_atoms, self.max_atoms, self.logger):
                return []

            # Convert NOMAD atoms data into an ASE Atoms object.
            atoms = create_atoms_object(atoms_data)

            if not validate_dimensionality(atoms, self.logger):
                return []
